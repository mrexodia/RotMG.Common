...
namespace RotMG.Common
{
    ...
    public class FameStats
    {
        ...
        {
            int num = 0;
            foreach (KeyValuePair<byte, Tuple<Func<FameStats, int>, Action<FameStats, int>>> current in FameStats.statList)
            {
                if (current.Value.Item1(this) != 0)
                {
                    num += 5;
                }
            }
            ...
            foreach (KeyValuePair<byte, Tuple<Func<FameStats, int>, Action<FameStats, int>>> current2 in FameStats.statList)
            {
                int num2 = current2.Value.Item1(this);
                if (num2 != 0)
                {
                    byteBuffer.WriteByte(current2.Key);
