...
namespace RotMG.Common.Networking.Packets
{
    ...
    public class PacketSHOOT : Packet
    {
        ...
        public static void InitIO()
        {
            ...
            {
                init.Field<byte>((PacketSHOOT p) => p.BulletId, FieldType.Byte).Field<int>((PacketSHOOT p) => p.OwnerId, FieldType.Int32).Field<ushort>((PacketSHOOT p) => p.ContainerType, FieldType.Int16).Field<Position>((PacketSHOOT p) => p.Position, FieldType.DataType).Field<float>((PacketSHOOT p) => p.Angle, FieldType.Float32).Field<short>((PacketSHOOT p) => p.Damage, FieldType.Int16).End();
            });
        }
    }
}
