...
namespace RotMG.Common.Networking.Packets
{
    ...
    public class PacketALLYSHOOT : Packet
    {
        ...
        public static void InitIO()
        {
            ...
            {
                init.Field<byte>((PacketALLYSHOOT p) => p.BulletId, FieldType.Byte).Field<int>((PacketALLYSHOOT p) => p.OwnerId, FieldType.Int32).Field<ushort>((PacketALLYSHOOT p) => p.ContainerType, FieldType.Int16).Field<float>((PacketALLYSHOOT p) => p.Angle, FieldType.Float32).End();
            });
        }
    }
}
