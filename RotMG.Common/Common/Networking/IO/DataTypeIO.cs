...
namespace RotMG.Common.Networking.IO
{
    ...
    public class DataTypeIO
    {
        ...
        static DataTypeIO()
        {
            Type[] types = typeof(DataTypeIO).Assembly.GetTypes();
            for (int i = 0; i < types.Length; i++)
            {
                ...
                if (!type.IsAbstract && !type.IsInterface && type.GetCustomAttributes(typeof(DataTypeAttribute), true).Length != 0)
                {
                    ...
                    if (method == null)
                    {
                        ...
                        Type type2 = typeof(DataTypeIO.IOData<>).MakeGenericType(new Type[]
                        {
                            type
                        });
                        Delegate value = Delegate.CreateDelegate(typeof(DataTypeIO.LoadFunc<>).MakeGenericType(new Type[]
                        {
                            type
                        }), method2);
                        type2.GetField("LoadFunc").SetValue(null, value);
                        Delegate value2 = Delegate.CreateDelegate(typeof(DataTypeIO.SaveFunc<>).MakeGenericType(new Type[]
                        {
                            type
                        }), method3);
                        type2.GetField("SaveFunc").SetValue(null, value2);
                        Delegate value3 = Delegate.CreateDelegate(typeof(DataTypeIO.SizeFunc<>).MakeGenericType(new Type[]
                        {
                            type
                        }), method4);
                        type2.GetField("SizeFunc").SetValue(null, value3);
