...
namespace RotMG.Common.Networking.IO
{
    ...
    public class InitDataTypeIO<T>
    {
        ...
        internal InitDataTypeIO()
        {
            ...
            this.load = new DynamicMethod("Load_" + this.typeT.FullName, this.typeT, new Type[]
            {
            ...
            this.size = new DynamicMethod("Size_" + this.typeT.FullName, typeof(int), new Type[]
            {
                this.typeT
            }, true);
        ...
        private void EmitLoad(MemberInfo member, FieldType type)
        {
            ...
            case FieldType.DataArray:
            {
                ...
                {
                    elementType = ((PropertyInfo)member).PropertyType.GetElementType();
                }
                else
                {
                    elementType = ((FieldInfo)member).FieldType.GetElementType();
                }
                this.loadGen.Emit(OpCodes.Ldarg_0);
                this.loadGen.Emit(OpCodes.Call, typeof(IOHelper).GetMethod("ReadDataArray").MakeGenericMethod(new Type[]
                {
                    elementType
                }));
                break;
            ...
            case FieldType.DataType:
            {
                ...
                if (member is PropertyInfo)
                {
                    type2 = ((PropertyInfo)member).PropertyType;
                }
                else
                {
                    type2 = ((FieldInfo)member).FieldType;
                }
                this.loadGen.Emit(OpCodes.Ldarg_0);
                this.loadGen.Emit(OpCodes.Call, typeof(DataTypeIO).GetMethod("Load").MakeGenericMethod(new Type[]
                {
                    type2
                }));
                break;
        ...
        private void EmitSave(MemberInfo member, FieldType type)
        {
            ...
            case FieldType.DataArray:
            {
                ...
                this.saveGen.Emit(OpCodes.Ldarg_0);
                action();
                this.saveGen.Emit(OpCodes.Call, typeof(IOHelper).GetMethod("WriteDataArray").MakeGenericMethod(new Type[]
                {
                    elementType
                }));
                return;
            ...
            case FieldType.DataType:
            {
                ...
                this.saveGen.Emit(OpCodes.Ldarg_0);
                action();
                this.saveGen.Emit(OpCodes.Call, typeof(DataTypeIO).GetMethod("Save").MakeGenericMethod(new Type[]
                {
                    type2
                }));
                return;
            ...
            default:
            {
                ...
                default:
                    ...
                    case FieldType.DataArray:
                    {
                        ...
                        this.sizeGen.Emit(OpCodes.Ldloc_0);
                        action();
                        this.sizeGen.Emit(OpCodes.Call, typeof(IOHelper).GetMethod("SizeOfDataArray").MakeGenericMethod(new Type[]
                        {
                            elementType
                        }));
                        this.sizeGen.Emit(OpCodes.Add);
                    ...
                    case FieldType.DataType:
                    {
                        ...
                        this.sizeGen.Emit(OpCodes.Ldloc_0);
                        action();
                        this.sizeGen.Emit(OpCodes.Call, typeof(DataTypeIO).GetMethod("SizeOf").MakeGenericMethod(new Type[]
                        {
                            type2
                        }));
                        this.sizeGen.Emit(OpCodes.Add);
